...
from pipeline_utils.cache import NpzCache, set_cache_device
...
@pipeline.add(deps=[], cache=NpzCache('step1a.npz'))
def step1a():
...
@pipeline.add(deps=[step1a], cache=NpzCache('step2.npz'))
def step2(step1: int):
    print(f'Running step 2... (received step1: {step1})')
    return step1 + 3


@pipeline.add(deps=[step1a, step2, step1b], cache=NpzCache('step3.npz'))
...
def main():
    opt = tyro.cli(Config)
    pipeline.set_global_cache_dir(opt.cache_dir)
